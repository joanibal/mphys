...
class ModalDecomp(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        if self.comm.rank == 0:
            ...
            self.freq = TACS.FrequencyAnalysis(
                self.tacs, sigma, mmat, kmat, self.gmres, num_eigs=self.nmodes, eig_tol=1e-12
            )
            self.freq.solve()
...
class ModalSolver(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input(
            "modal_stiffness", shape=input_shape, src_indices=src_indices, val=np.ones(nmodes), desc="modal stiffness"
        )
...
class ModalForces(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input("mode_shape", shape=input_shape, src_indices=src_indices, desc="structural mode shapes")
    ...
    def compute(self, inputs, outputs):
        if self.comm.rank == 0:
            outputs["mf"][:] = 0.0
            for imode in range(self.nmodes):
                outputs["mf"][imode] = np.sum(
                    inputs["mode_shape"][imode * self.mode_size : (imode + 1) * self.mode_size] * inputs["f_s"][:]
                )
                ...
                if "mf" in d_outputs:
                    if "f_s" in d_inputs:
                        for imode in range(self.options["nmodes"]):
                            d_outputs["mf"][imode] += np.sum(
                                inputs["mode_shape"][imode * self.mode_size : (imode + 1) * self.mode_size]
                                * d_inputs["f_s"][:]
                            )
            if mode == "rev":
                if "mf" in d_outputs:
...
class ModalDisplacements(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        if self.comm.rank == 0:
            ...
            for imode in range(self.nmodes):
                outputs["u_s"][:] += (
                    inputs["mode_shape"][imode * self.mode_size : (imode + 1) * self.mode_size] * inputs["z"][imode]
                )
...
class ModalGroup(om.Group):
    ...
    def setup(self):
        ...
        self.add_subsystem(
            "modal_forces", ModalForces(nmodes=nmodes, mode_size=mode_size), promotes_inputs=["mode_shape", "f_s"]
        )
        self.add_subsystem("modal_solver", ModalSolver(nmodes=nmodes), promotes_inputs=["modal_stiffness"])

        self.add_subsystem(
            "modal_disps",
            ModalDisplacements(nmodes=nmodes, mode_size=mode_size),
...
class ModalBuilder(object):
    ...
    def get_mesh_element(self):
        return ModalDecomp(struct_solver=self.solver, ndv=self.solver_dict["ndv"], nmodes=self.nmodes)
