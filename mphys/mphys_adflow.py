...
class AdflowMesh(ExplicitComponent):
    ...
    def initialize(self):
        ...
        self.options.declare('groupName')
    ...
    def setup(self):
        ...
        if self.options['groupName']:
            famGroup = self.options['groupName']
            self.x_a_fam = self.aero_solver.mesh.getSurfaceCoordinates(famGroup).flatten(order='C')
            self.add_output('x_a_fam', shape=self.x_a_fam,size, desc='surface of the given family group' )
    ...
    def compute(self,inputs,outputs):
        if 'x_a0_points' in inputs:
            ...
            outputs['x_a0_surface'] = self.x_a_fam
...
class Geo_Disp(ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('x_a0',shape_by_connect = True, src_indices_by_connect = True, desc='aerodynamic surface with geom changes')
        self.add_input('u_a', shape_by_connect = True, src_indices_by_connect = True, desc='aerodynamic surface displacements')
...
class AdflowSolver(ImplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('x_g', shape=local_coord_size, src_indices_by_connect = True)

        self.add_output('q', shape=local_state_size)

        #self.declare_partials(of='q', wrt='*')

    #     # if the solver component didn't share a solver object where the mesh was already read and set
    #     #  you could do something like this 
    #     self.add_output('q', determine_shape=True)

    #     with a determine_shapes that looks like this

    # def determine_shape(self, var_meta_data):
    #     solver.mesh.setSolverGrid(var_meta_data['x_g'].val)
    #       self.set_variable_shape('q', solver.getStateSize())
...
class AdflowForces(ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('x_g', shape_by_connect = True, src_indices_by_connect = True,)
        self.add_input('q', shape_by_connect = True, src_indices_by_connect = True,)
