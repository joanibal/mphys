...
class RLT_disp_xfer(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        # Outputs
        self.add_output(
            "u_a", shape=total_dof_aero, val=np.zeros(total_dof_aero), desc="Aerodynamic surface displacements"
        )
        ...
        if mode == "rev":
            if "u_a" in d_outputs:
                ...
                if "x_a0" in d_inputs:
                    ...
                    self.transfer.zeroReverseSeeds()
                    if self.isStruct:
                        self.transfer.setDisplacementsSens(self.ustruct, self.struct_seed, d_outputs["u_a"])
...
class RLT_load_xfer(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input(
            "f_a", shape=total_dof_aero, src_indices=np.arange(ax1, ax2, dtype=int), desc="Aerodynamic force vector"
        )

        # Outputs
        self.add_output("f_s", shape=total_dof_struct, desc="structural force vector")
        ...
        if mode == "rev":
            if "f_s" in d_outputs:
                ...
                if "f_a" in d_inputs:
                    ...
                    self.transfer.addAeroForcesSens(np.ravel(inputs["f_a"]), np.ravel(f_ab), self.struct_seed)
                    d_inputs["f_a"] = -f_ab
                ...
                if "x_a0" in d_inputs:
                    ...
                    self.transfer.zeroReverseSeeds()
                    self.transfer.addAeroForcesSens(inputs["f_a"], tmp, self.struct_seed)
...
class RLT_builder(object):
    ...
    def init_xfer_object(self, comm):
        ...
        self.xfer_object = SimpleLDTransfer(aero_solver, dummy_pytacs, comm=comm, options=self.options)
