...
class MeldObjBuilder(ObjBuilder):
    ...
    def build_obj(self, comm):
        ...
        meld = TransferScheme.pyMELD(
            comm, comm, 0, comm, 0, self.options["isym"], self.options["n"], self.options["beta"]
        )
...
class MeldThermalObjBuilder(ObjBuilder):
    ...
    def get_obj(self, comm):
        ...
        if not self.obj_built:
            ...
            self.meld_therm = TransferScheme.pyMELDThermal(
                comm, comm, 0, comm, 0, self.options["isym"], self.options["n"], self.options["beta"]
            )
...
class MELD_disp_xfer(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        # inputs
        self.add_input(
            "x_s0", shape_by_conn=True, desc="initial structural node coordinates"
        )  # np.arange(sx1, sx2, dtype=int)
        self.add_input(
            "x_a0", shape_by_conn=True, desc="initial aerodynamic surface node coordinates"
        )  # np.arange(ax1, ax2, dtype=int)
        self.add_input(
            "u_s", shape_by_conn=True, desc="structural node displacements"
        )  # np.arange(su1, su2, dtype=int)
