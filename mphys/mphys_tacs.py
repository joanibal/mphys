...
class TacsSolver(om.ImplicitComponent):
    ...
    def setup(self):
        ...
        # inputs
        self.add_input(
            "x_s0", val=xpts_arr, src_indices=np.arange(n1, n2, dtype=int), desc="structural node coordinates"
        )
...
class TacsWriteSolution(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input(
            "u",
            shape=state_size,
            src_indices=np.arange(s1, s2, dtype=int),
            desc="state vector, e.g. displacements for temperatures",
        )
...
class TacsFunctions(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input(
            "x_s0", shape=node_size, src_indices=np.arange(n1, n2, dtype=int), desc="structural node coordinates"
        )
...
class PrescribedLoad(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        if not "load_function" in self.options["solver_options"]:
            raise KeyError(
                "`load_function` must be supplied in the solver options dictionary"
    ...
    def __init__(self, **kwargs):
        ...
        for comp in self.group_components:
            if self.group_options[comp]:
                ...
                self.add_subsystem(
                    comp,
                    self.group_components[comp](solver_options=solver_options, check_partials=self.check_partials),
                    promotes=["*"],
