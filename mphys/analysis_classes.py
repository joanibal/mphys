    ...
    def setup(self):
        ...
        if self.aero_discipline and self.struct_discipline:
            ...
            if self.xfer_builder is None:
                raise MPHYS_Error(
                    "MPHYS_Multipoint group requires a transfer builder to couple aerodynamic and structural analyses."
                )
...
class CoupledAnalysis(om.Group):
    ...
    def setup(self):
        ...
        # use the builders to create the componenets of the analysis
        for discipline in self.options["builders"]:
            builder = self.options["builders"][discipline]

            # build the object if not done so.
            if not builder.object_built:
                builder.build_object(self.comm)

            # add any componets to this analysis
            for name, component in builder.get_component():

                name = discipline + name

                print("adding {} subsystem to coupled analysis".format(name))
